...
if (isset($_GET['q'])) {
    ...
    $nombre = $_GET['pro'];
    $hoy = date('Y-m-d');
    $producto = mysqli_query($conexion, "SELECT * FROM producto WHERE codigo LIKE '%" . $nombre . "%' OR NombreProducto LIKE '%" . $nombre . "%' AND vencimiento > '$hoy' OR vencimiento = '0000-00-00'");
    while ($row = mysqli_fetch_assoc($producto)) {
        $data['id'] = $row['id'];
        $data['label'] = $row['codigo'] . ' - ' .$row['NombreProducto'];
        $data['value'] = $row['NombreProducto'];
    ...
    $id = $_SESSION['idUser'];
    $datos = array();
    $detalle = mysqli_query($conexion, "SELECT d.*, p.id, p.NombreProducto FROM detalle_temp d INNER JOIN producto p ON d.id_producto = p.id WHERE d.id_usuario = $id");
    while ($row = mysqli_fetch_assoc($detalle)) {
        $data['id'] = $row['id'];
        $data['NombreProducto'] = $row['NombreProducto'];
    ...
    if ($insertar) {
        ...
        while ($row = mysqli_fetch_assoc($consultaDetalle)) {
            ...
            $stockActual = mysqli_query($conexion, "SELECT * FROM producto WHERE id = $id_producto");
            $stockNuevo = mysqli_fetch_assoc($stockActual);
            $stockTotal = $stockNuevo['existencia'] - $cantidad;
            $stock = mysqli_query($conexion, "UPDATE producto SET existencia = $stockTotal WHERE id = $id_producto");
        } 
        if ($insertarDet) {
    ...
    $id = $_GET['id'];
    $sql = mysqli_query($conexion, "SELECT * FROM producto WHERE id = $id");
    $data = mysqli_fetch_array($sql);
    echo json_encode($data);
    exit;
